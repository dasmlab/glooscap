...
# Verify service account is in install.yaml
if ! grep -q "kind: ServiceAccount" "${OPERATOR_DIR}/dist/install.yaml"; then
    log_error "ServiceAccount not found in dist/install.yaml!"
    log_info "This should be generated by kustomize from config/rbac/service_account.yaml"
    exit 1
fi
